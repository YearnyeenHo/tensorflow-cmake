...
std::vector<Peak> peak_infos_line;

int roundpaf(float v);
std::vector<VectorXY> get_paf_vectors(std::vector<float>&pafmap, const int& ch_id1, const int& ch_id2, int& f2, int& f3, Peak& peak1, Peak& peak2);
...
//    const double THRESH_PAF = 0.40;
    std::vector<Peak> peak_infos[NUM_PART];
    int peak_cnt = 0;
    ...
    // Start to Connect
    std::vector<Connection> connection_all[COCOPAIRS_SIZE];
    for (int pair_id = 0; pair_id < COCOPAIRS_SIZE; pair_id ++) {
        std::vector<ConnectionCandidate> candidates;
        std::vector<Peak>& peak_a_list = peak_infos[COCOPAIRS[pair_id][0]];
        std::vector<Peak>& peak_b_list = peak_infos[COCOPAIRS[pair_id][1]];
        ...
        for (int peak_a_id = 0; peak_a_id < (int) peak_a_list.size(); peak_a_id ++) {
            ...
            for (int peak_b_id = 0; peak_b_id < (int) peak_b_list.size(); peak_b_id ++) {
                ...
                std::vector<VectorXY> paf_vecs = get_paf_vectors(pafmap, COCOPAIRS_NET[pair_id][0], COCOPAIRS_NET[pair_id][1], f2, f3, peak_a, peak_b);
                float scores = 0.0f;
                ...
                float criterion2 = scores / STEP_PAF + std::min(0.0, 0.5 * h1 / norm - 1.0);
        ...
        std::vector<Connection>& conns = connection_all[pair_id];
